...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "anac-pl-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ANAC_PL_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ANAC_PL_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "anac-pl-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/anac-pl-pp-cli/feedback.jsonl.
When ` + "`ANAC_PL_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ANAC_PL_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  anac-pl-pp-cli feedback list
  anac-pl-pp-cli feedback list --limit 5
  anac-pl-pp-cli feedback list --json`,
