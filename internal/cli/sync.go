...
Resource scoping:
  ...
  anac-pl-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  anac-pl-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  anac-pl-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  anac-pl-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  anac-pl-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/anac-pl-pp-cli/data.db)")
...
var pageItemKeys = []string{
	...
	"Data", "Results", "Items", "Records", "Nodes", "Entries", "Features",
	// ANAC /avvisi-full-text wraps the list under "content"; keep it here so
	// sync unwraps the page instead of treating the whole envelope as one row.
	"content", "Content",
}
